package com.agx.jetpackmvvmtest.ui.fragment.login

import android.app.Application
import androidx.lifecycle.MutableLiveData
import com.agx.jetpackmvvm.base.viewmodel.BaseViewModel
import com.agx.jetpackmvvmtest.R
import com.agx.jetpackmvvmtest.extend.isPhone
...
class LoginViewModel(application: Application, private val loginRepository: LoginRepository) :
    ...
    fun checkInputData() {
        if (userPhone.value?.isPhone() == true && (loginEnum == LoginEnum.PHONE_LOGIN || (password.value?.length
                ?: 0) >= 8)
        ) {
