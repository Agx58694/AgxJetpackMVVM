...
abstract class BaseVmDbFragment<VM : BaseViewModel, DB : ViewDataBinding> : Fragment() {
    ...
    private fun registerDefUIChange() {
        mViewModel.loadingChange.showDialog.collectIn(viewLifecycleOwner) {
            showLoading(it)
        }
        mViewModel.loadingChange.dismissDialog.collectIn(viewLifecycleOwner) {
            dismissLoading()
        }
        mViewModel.layoutDataChange.layoutDataLoading.collectIn(viewLifecycleOwner) {
            layoutDataLoading()
        }
        mViewModel.layoutDataChange.layoutDataError.collectIn(viewLifecycleOwner) {
            layoutDataError(it)
        }
        mViewModel.layoutDataChange.layoutDataTimeout.collectIn(viewLifecycleOwner) {
            layoutDataTimeout()
        }
    }
}